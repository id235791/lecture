...
public class ProductController {
	...
	public ArrayList<ProductDTO> search(String keyword) {
		...
		if(filter.contains(keyword)) {
			...
			//데이터 로직
			ArrayList<ProductDTO> result = pdao.findProducts(keyword);
			return result;
		}
	}
	...
	public ProductDTO getDetail(int prodnum) {
		ProductDAO pdao = new ProductDAO();
		return pdao.findProductByProdnum(prodnum);
	}

	public void updateLikeCnt(int prodnum) {
		ProductDAO pdao = new ProductDAO();
		pdao.updateLikeCnt(prodnum);
	}

	public String getSellerPhone(String userid) {
		UserDAO udao = new UserDAO();
		return udao.findUserById(userid).getUserphone();
	}
}
