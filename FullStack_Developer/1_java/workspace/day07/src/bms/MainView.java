...
public class MainView {
	public MainView() {
		...
		int money = 0;
		//BankManger 클래스에 있는 session 객체를 편하게 쓰기 위해 따로 변수에 담기
		...
		while(true) {
			...
			if(choice == 4) {
				...
				//로그아웃이기 때문에 session 초기화
				//지역변수 session은 MainView 생성자 호출이 끝남과 동시에 자동으로 사라지므로
				//실제 로그인 정보를 저장해놓은 BankManger.session을 초기화한다.
				BankManager.session = null;
				break;
			}
			//session 객체는 업캐스팅 하며 저장했던 객체를 꺼내서 담아준 것이므로 업캐스팅 되어있는 상태이다.
			//따라서 오버라이딩 한 메소드는 새로운 내용으로 수행하게 된다.
