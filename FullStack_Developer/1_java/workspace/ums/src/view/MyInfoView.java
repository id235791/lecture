...
public class MyInfoView {
	public MyInfoView() {
		...
		//세션에 있는 정보 띄워주기
		System.out.println("======"+loginUser.getUsername()+"님의 회원정보======");
		System.out.println("아이디 : "+loginUser.getUserid());
		System.out.println("비밀번호 : "+loginUser.getUserpw());
		System.out.println("핸드폰 번호 : "+loginUser.getUserphone());
		...
		if(choice == 4) {
			...
			if(loginUser.getUserpw().equals(userpw)) {
				//회원탈퇴 요청
				if(controller.leaveId(loginUser.getUserid())) {
			...
			if(controller.modifyUser(loginUser.getUserid(), choice, newData)) {
