				...
				<c:when test="${files != null and files.size()>0 }">
					<c:forEach var="i" begin="0" end="${files.size()-1}">
						...
						<tr>
							<th>첨부파일${i+1}</th>
							<td>
								<a href="${cp}/board/file?systemname=${file.systemname}&orgname=${file.orgname}">${file.orgname}</a>
							</td>
						</tr>
				...
				<tr>
					<td>
						<c:if test="${board.userid == loginUser}">
							<a class="btn" href="${cp}/board/update?boardnum=${board.boardnum}&page=${param.page}&keyword=${param.keyword}">수정</a>
							<a class="btn" href="${cp}/board/delete?boardnum=${board.boardnum}&page=${param.page}&keyword=${param.keyword}">삭제</a>
						</c:if>
									...
									<td>
										...
										<c:if test="${reply.userid == loginUser}">
											<div class="btns">
												...
												<a class="btn" href="javascript:updateReplyOk(${i},${reply.replynum})" style="display:none;" id="end${i}">수정완료</a>
												<a class="btn" href="javascript:deleteReply(${reply.replynum})">삭제</a>
											</div>
										</c:if>
									</td>
								</tr>
