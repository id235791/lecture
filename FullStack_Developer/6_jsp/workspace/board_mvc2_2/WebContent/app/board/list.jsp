...
<body class="list">
	...
	<div id="wrap" class="list">
		...
		<!-- 게시글 리스트 띄워주는 테이블 -->
		<div class="tar w900">글 개수 : ${totalCnt}</div>
					...
					<c:when test="${list != null and list.size() > 0}">
						<c:forEach var="i" begin="0" end="${list.size() - 1}">
							...
							<div class="row">
								...
								<div>
									<c:if test="${hot_board[i] == 'O'}"><sup class="hot">Hot</sup></c:if>
									<a href="${cp}/board/view?boardnum=${board.boardnum}&page=${page}&keyword=${keyword}">
										${board.boardtitle} <span id="reply_cnt">[${reply_cnt_list[i]}]</span>
									</a>
								</div>
				...
				<tr>
					<td>
						<c:if test="${startPage != 1}">
							<a class="btn" href="${cp}/board/list?page=${startPage-1}&keyword=${keyword}">&lt;</a>
						</c:if>
						<c:forEach begin="${startPage}" end="${endPage}" var="i">
							<c:choose>
								...
								<c:otherwise>
									<a class="btn" href="${cp}/board/list?page=${i}&keyword=${keyword}">${i}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${endPage != totalPage}">
							<a class="btn" href="${cp}/board/list?page=${endPage+1}&keyword=${keyword}">&gt;</a>
						</c:if>
					</td>
				</tr>
			</tbody>
